...
def run(args, log_interval=5000, rerun=False):
    ...
    meta_optimiser = optim.Adam(model_outer.weights + model_outer.biases + [model_outer.task_context],
                                args.lr_meta)
    hsml_optimiser = optim.Adam(hsml_model.parameters(), args.lr_meta)
    # initialise loggers
    logger = Logger()
    ...
    for i_iter in range(args.n_iter):
        ...
        hsml_gradient = [0 for _ in range(len(hsml_model.state_dict()))]
        ...
        for t in range(args.tasks_per_metaupdate):
            ...
            train_inputs = torch.cat([x,y],dim=1).unsqueeze(dim=0)
            gate_p,mse_loss = hsml_model(train_inputs)
            
            
            model_inner.gate(gate_p)
            
            for _ in range(args.num_inner_updates):

                # make prediction using the current model
                outputs = model_inner(x)

                # get targets
                targets = target_functions[t](x)
            ...
            loss_meta = loss_meta + 0.2 * mse_loss
            # compute gradient w.r.t. *outer model*
            task_grads = torch.autograd.grad(loss_meta,
                                             model_outer.weights + model_outer.biases + [model_outer.task_context], retain_graph=True)
            
            for i in range(len(model_inner.weights + model_inner.biases) + 1):
                meta_gradient[i] += task_grads[i].detach()
            
            task_grads = torch.autograd.grad(loss_meta,
                                             hsml_model.parameters())
            for i in range(len(hsml_model.state_dict())):
                hsml_gradient[i] += task_grads[i].detach()
        # ------------ meta update ------------

        meta_optimiser.zero_grad()
        hsml_optimiser.zero_grad()
        # print(meta_gradient)
        ...
        for i, param in enumerate(hsml_model.parameters()):
            param.grad = hsml_gradient[i] / args.tasks_per_metaupdate
        ...
        if i_iter % log_interval == 0:
            ...
            loss_mean, loss_conf = eval(args, copy.copy(model_outer), task_family=task_family_train,
                                        num_updates=args.num_inner_updates,encoder = copy.copy(hsml_model))
...
def eval(args, model, task_family, num_updates, n_tasks=100, return_gradnorm=False, encoder=False):
    # copy weights of network
    ...
    for t in range(n_tasks):
        ...
        train_inputs = torch.cat([curr_inputs,curr_targets],dim=1).unsqueeze(dim=0)
        gate_p,_ = encoder(train_inputs)
        model.gate(gate_p)
        # ------------ update on current task ------------

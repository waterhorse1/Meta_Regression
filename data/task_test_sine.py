...
class sine:
    ...
    def sample_task(self):
        ...
        return self.get_target_function(amplitude, phase, w)

    @staticmethod
    def get_target_function(amplitude, phase, w):
        def target_function(x):
            if isinstance(x, torch.Tensor):
                return torch.sin(w * x) * amplitude + phase
            else:
                return np.sin(w * x - phase) * amplitude + phase
